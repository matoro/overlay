...
SRC_URI="https://github.com/arkime/arkime/archive/refs/tags/v${PV}.tar.gz -> ${P}.tar.gz
	viewer? ( https://github.com/gchq/CyberChef/releases/download/v9.54.0/CyberChef_v9.54.0.zip )"
...
DEPEND="
	...
	viewer? ( >=net-libs/nodejs-16.16.0[npm] <net-libs/nodejs-18:=[npm] )
	parliament? ( >=net-libs/nodejs-16.16.0[npm] )
	wise? ( >=net-libs/nodejs-16.16.0[npm] )
"

RDEPEND="${DEPEND}
	capture? (
		>=dev-libs/glib-2.68.3:2
	...
	viewer? ( >=net-libs/nodejs-16.16.0 <net-libs/nodejs-18:= )
	parliament? ( >=net-libs/nodejs-16.16.0 )
	wise? ( >=net-libs/nodejs-16.16.0 )
"
...
src_prepare() {
	...
	if use capture; then
		...
		# sort this out of anybody cares about s3 plugin
		sed -i 's/npm install --production/true/g' capture/plugins/Makefile.in || die
