...
SRC_URI="https://github.com/arkime/arkime/archive/refs/tags/v${PV}.tar.gz -> ${P}.tar.gz
	viewer? ( https://github.com/gchq/CyberChef/releases/download/v10.19.0/CyberChef_v10.19.0.zip )"

LICENSE="Apache-2.0"
SLOT="0"
KEYWORDS="~amd64 ~arm ~arm64 ~ppc64 ~x86"
IUSE="+capture +viewer +parliament +wise +cont3xt pfring daq test netlink lua kafka"
REQUIRED_USE="lua? ( ${LUA_REQUIRED_USE} ) pfring? ( capture ) daq? ( capture ) netlink? ( capture ) lua? ( capture ) kafka? ( capture )"
...
DEPEND="
	...
	viewer? ( >=net-libs/nodejs-18.20.3[npm] <net-libs/nodejs-19:=[npm] )
	parliament? ( >=net-libs/nodejs-18.20.3[npm] )
	wise? ( >=net-libs/nodejs-18.20.3[npm] )
"

RDEPEND="${DEPEND}
	capture? (
		>=dev-libs/glib-2.72.4:2
		>=app-forensics/yara-4.2.3:=
	...
	viewer? ( >=net-libs/nodejs-18.20.3 <net-libs/nodejs-19:= )
	parliament? ( >=net-libs/nodejs-18.20.3 )
	wise? ( >=net-libs/nodejs-18.20.3 )
"
...
src_prepare() {
	...
	if use capture; then
		...
		sed -i "s/\"rdkafka.h\"/\"librdkafka\/rdkafka.h\"/g" "capture/plugins/kafka/kafka.c" || die
...
src_compile() {
	if use capture; then
		...
		use kafka && emake -C capture/plugins/kafka
	fi
