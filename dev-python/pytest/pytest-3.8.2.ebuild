...
COMMON_DEPEND="
	...
	$(python_gen_cond_dep '>=dev-python/pathlib2-2.2.0[${PYTHON_USEDEP}]' python3_5)
	doc? (
...
python_prepare_all() {
	...
	# broken tests per matoro 2019/06/13
	rm testing/test_{assertion,config}.py || die
	rm testing/python/{approx,raises}.py || die
